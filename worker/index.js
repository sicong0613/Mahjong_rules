...
//   POST /api/recognize  body:图片base64          → 拍照识别代理（转发 Roboflow，隐藏密钥）
...
// 拍照识别（Roboflow 代理）
const ROBOFLOW_MODEL          = 'riichi-mahjong-detection/3'; // Roboflow model_id
const ROBOFLOW_CONFIDENCE     = 20;   // 置信度阈值(%)，低一些以多召回，配合人工确认
const ROBOFLOW_OVERLAP        = 50;   // NMS 重叠阈值(%)，密排牌调高以保住相邻框
const RECOGNIZE_LIMIT_PER_HOUR = 40;  // 同一 IP 每小时最多识别次数
const MAX_IMAGE_B64_LEN       = 8_000_000; // base64 图片体积上限（约 6MB 原图）
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path.endsWith('/api/recognize')) {
        if (request.method === 'POST') return await recognize(request, env);
      }
...
// ── POST /api/recognize ────────────────────────────────────────────
// 拍照识别代理：前端传图片 base64，服务端带密钥转发给 Roboflow，隐藏 API key。
// body 为图片的 base64 字符串（不含 data: 前缀）。
async function recognize(request, env) {
  if (!env.ROBOFLOW_API_KEY)
    return json({ error: 'ROBOFLOW_API_KEY 未在 Worker 配置（请加 Secret 后重新部署）' }, 500);

  const ip = request.headers.get('CF-Connecting-IP') || 'unknown';
  const rlKey = `rf:${ip}`;
  const rl = (await env.KV.get(rlKey, 'json')) || { n: 0, exp: Date.now() + 3_600_000 };
  if (Date.now() > rl.exp) { rl.n = 0; rl.exp = Date.now() + 3_600_000; }
  if (rl.n >= RECOGNIZE_LIMIT_PER_HOUR) return json({ error: 'rate limited' }, 429);
  rl.n++;
  await env.KV.put(rlKey, JSON.stringify(rl), { expirationTtl: 3600 });

  const b64 = (await request.text()).trim();
  if (b64.length < 100)               return json({ error: 'empty image' }, 400);
  if (b64.length > MAX_IMAGE_B64_LEN)  return json({ error: 'image too large' }, 413);

  const url = `https://serverless.roboflow.com/${ROBOFLOW_MODEL}`
    + `?api_key=${encodeURIComponent(env.ROBOFLOW_API_KEY)}`
    + `&confidence=${ROBOFLOW_CONFIDENCE}&overlap=${ROBOFLOW_OVERLAP}&format=json`;
  ...
  } catch (e) { return json({ error: 'roboflow fetch failed: ' + e.message }, 502); }

  const data = await rf.json().catch(() => null);
  if (!rf.ok || !data) return json({ error: `roboflow error (HTTP ${rf.status})` }, 502);
  return json(data);   // 原样返回 {predictions:[...], image:{...}}
}
