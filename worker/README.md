...
| `PUT`    | `/api/fan-logs/import?token=&mode=` | 管理员 | 批量导入逐副记录（完整记录）。body 为「副」数组，每副 `["平和","断幺"]` 或 `{fans,ts,player,ip,tiles,geo}`（字段可省略/null）。默认追加；`mode=replace` 先清空全部记录再导入。随后重算计数 |
